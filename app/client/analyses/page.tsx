...
import { Search, Filter, TrendingUp, Zap, ChevronDown, ChevronUp, X } from 'lucide-react'
              ...
              <motion.div key={a.id}
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2.5">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xs font-bold flex-shrink-0 transition-colors group-hover:bg-opacity-20"
                      style={{ background: 'rgba(212,175,55,0.12)', color: '#D4AF37' }}>
                      {a.ticker.slice(0, 4)}
                    </div>
                    <div>
                      <div className="font-bold text-sm" style={{ color: '#F5F5F5' }}>{a.ticker}</div>
                      <div className="text-[11px]" style={{ color: '#5C5C5C' }}>{a.market} · {a.timeframe}</div>
                    </div>
                  </div>
                  {sig && <span className={`${sig.cls} text-[10px] font-bold px-2 py-0.5 rounded flex-shrink-0`}>{sig.label}</span>}
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {rr && (
                      <span className="text-[10px]" style={{ color: '#5C5C5C' }}>
                        R/R: 1:{rr}
                      </span>
                    )}
                  </div>
