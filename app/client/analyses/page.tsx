...
export default function ClientAnalysesPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('client-analyses')
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'technical_analyses' }, () => fetchAnalyses())
  ...
  const filtered = analyses.filter(a => {
    const matchSearch = a.title.toLowerCase().includes(search.toLowerCase()) || a.ticker.toLowerCase().includes(search.toLowerCase())
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-3">
            ...
            return (
              <button key={s} onClick={() => setSignal(s)}
                ...
                style={{
                  ...
                  color: signal === s ? (s === 'buy' ? '#00C853' : s === 'sell' ? '#FF1744' : s === 'hold' ? '#2196F3' : '#D4AF37') : '#707070',
        ...
        <select value={market} onChange={e => setMarket(e.target.value)} className="input-premium w-40">
          ...
          {filtered.map((a, i) => {
            ...
            return (
              <motion.div key={a.id}
                ...
                {a.chart_image_url && (
                  <div className="w-full h-32 overflow-hidden" style={{ borderBottom: '1px solid var(--noir-border)' }}>
                    <img src={a.chart_image_url} alt="Graphique"
...
function AnalysisDetailModal({ analysis: a, current, onClose }: { analysis: TechnicalAnalysis; current?: number; onClose: () => void }) {
  ...
  function ouvrirPosition() {
    const params = new URLSearchParams({
      ticker:  a.ticker,
      note:    a.title,
      ...(an.support && { support: an.support.toString() }),
      ...(an.r1      && { r1:      an.r1.toString()      }),
      ...(an.r2      && { r2:      an.r2.toString()      }),
      ...(an.r3      && { r3:      an.r3.toString()      }),
      from_analyse: a.id,
    })
    onClose()
    router.push(`/client/positions?nouvelle=1&${params.toString()}`)
  }
      ...
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95 }}
        ...
        <div className="px-6 py-4 border-b flex items-center justify-between"
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="p-1.5 rounded-lg" style={{ color: '#5C5C5C' }}><X size={16} /></button>
          </div>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          {hasNiveaux && (
            <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--noir-border)' }}>
              <div className="px-4 py-2.5 text-xs font-semibold tracking-wider"
                style={{ background: 'rgba(255,255,255,0.02)', color: '#5C5C5C', borderBottom: '1px solid var(--noir-border)' }}>
                NIVEAUX TECHNIQUES
              </div>
              {[
                an.r3      && { label: 'Résistance 3', value: an.r3,      color: '#D4AF37' },
                an.r2      && { label: 'Résistance 2', value: an.r2,      color: '#00C853' },
                an.r1      && { label: 'Résistance 1', value: an.r1,      color: '#00C853' },
                an.support && { label: 'Support',      value: an.support, color: '#FF1744' },
              ].filter(Boolean).map((n: any) => (
                <div key={n.label} className="flex items-center justify-between px-4 py-3 border-b last:border-0"
                  style={{ borderColor: 'var(--noir-border)' }}>
                  <span className="text-sm" style={{ color: '#707070' }}>{n.label}</span>
                  <span className="text-sm font-bold font-mono" style={{ color: n.color }}>
                    {n.value?.toLocaleString('fr-TN', { minimumFractionDigits: 3 })}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {risk && (
              <div className="p-3 rounded-xl flex items-center gap-3"
                style={{ background: 'var(--noir-elevated)', border: '1px solid var(--noir-border)' }}>
                <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: `${risk.color}18` }}>
          ...
          {/* Description */}
          <div>
            <div className="text-xs font-semibold mb-2 tracking-wider" style={{ color: '#5C5C5C' }}>ANALYSE ET COMMENTAIRES</div>
          ...
          {a.chart_image_url && (
            <div>
              <div className="text-xs font-semibold mb-2 tracking-wider" style={{ color: '#5C5C5C' }}>GRAPHIQUE TECHNIQUE</div>
              <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--noir-border)' }}>
                <img src={a.chart_image_url} alt="Analyse graphique" className="w-full object-cover" />
              </div>
            </div>
          )}
