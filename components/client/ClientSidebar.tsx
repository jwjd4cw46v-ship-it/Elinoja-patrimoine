    ...
    >
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'8px' }}>
        <div>
          ...
          <div style={{ fontSize:'9px', color:'#3A3A3A', marginTop:'1px', maxWidth:'90px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
            {item.name}
          </div>
        </div>
        <div style={{ textAlign:'right' }}>
          <div style={{
            ...
            color: item.current === 0 ? '#3A3A3A' : isBelowLow ? '#FF3B3B' : isAboveHigh ? '#00C853' : '#F5F5F5',
...
export default function ClientSidebar({ profile }: { profile: Profile }) {
  ...
  const activeAlerts = items.filter(
    i => i.current > 0 && (
      (i.low  > 0 && i.current < i.low) ||
      (i.high > 0 && i.current > i.high)
    )
  ).length
