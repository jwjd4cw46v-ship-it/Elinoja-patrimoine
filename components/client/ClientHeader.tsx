...
export default function ClientHeader({ profile }: { profile: Profile }) {
  ...
  async function fetchMarkets() {
    try {
      ...
      // Récupérer les seuils d'alerte depuis Supabase
  ...
  function handleBellOpen() {
    setNotifOpen(o => !o)
    if (!notifOpen) {
      // Réinitialise les compteurs à l'ouverture
      setCountLow(0)
      setCountHigh(0)
    }
  }
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <div className="relative">
            <button
              ...
              {/* Point d'activité par défaut (remplacé par badges si alertes) */}
              {totalAlerts === 0 && (
                <span
              ...
              {/* Badge rouge — franchissements BAS (bas gauche) */}
              {countLow > 0 && (
                <span
              ...
              {/* Badge vert — franchissements HAUT (bas droite) */}
              {countHigh > 0 && (
                <span
